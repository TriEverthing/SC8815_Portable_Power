...
/* include ESP32-IDF driver driver */
...
disp_backlight_h disp_backlight_new(const disp_backlight_config_t *config)
{
    ...
    if (config->pwm_control){
        ...
        //rom_gpio_matrix_out(config->gpio_num, ledc_periph_signal[LEDC_LOW_SPEED_MODE].sig_out0_idx + config->channel_idx, config->output_invert, 0);
    ...
    else
    {
        ...
        rom_gpio_pad_select_gpio(config->gpio_num);
        ESP_ERROR_CHECK(gpio_set_direction(config->gpio_num, GPIO_MODE_OUTPUT));
        rom_gpio_matrix_out(config->gpio_num, SIG_GPIO_OUT_IDX, config->output_invert, false);
    }
